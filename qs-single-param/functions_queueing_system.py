...
def update_pi_fixedparam(pi_curr, p_theta, o_new_ind, numvals_z, dep_rate=None):
...
def update_pi_new(pi_curr, theta_t, gamma, omega_t, theta_indices, theta_not_indices, M, K,\
                  p_theta, o_new_ind, dep_rate):
    
    p_th_pert = compute_p_theta(p_theta, theta_t+gamma*np.sin(omega_t), theta_indices, theta_not_indices, M, K)
...
def compute_ll_next(ll_prev, pi_current, p_theta_current, o_new_ind, dep_rate):
    # p_theta_reduced = p_theta_current[:, o_new_ind, :, u_old_ind].T #shape xn,xnpl1
    control_policy = np.array([1-dep_rate[o_new_ind],dep_rate[o_new_ind]]).reshape((1,1,-1))
    p_theta_reduced = np.sum(p_theta_current[:, o_new_ind, :, :]*control_policy, axis=-1).T
...
def compute_log_likelihood_alldata(theta_estimate, O_t, Z_t, T, z_vals, o_vals, x_vals, K, saveLLt=False, dep_rate=None):
    ...
    for t in range(1,T):
        ...
        LL = compute_ll_next(ll_prev=LL, pi_current=pi_t, p_theta_current=p_theta, o_new_ind=ot_ind, dep_rate=dep_rate)
        pi_t = update_pi_fixedparam(pi_t, p_theta, ot_ind, numvals_z, dep_rate=dep_rate)
        LL_t[t] = LL
    if saveLLt:
        return LL_t
    else:
