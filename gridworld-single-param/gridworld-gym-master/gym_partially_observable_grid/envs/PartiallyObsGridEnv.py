...
class PartiallyObservableWorld(gym.Env):
    def __init__(self,
        ...
        # Available actions
        self.actions_dict = {'up': 0, 'down': 1, 'left': 2, 'right': 3, 'none': -1}
        self.action_space_to_act_map = {i:k for k,i in self.actions_dict.items()}
        self.actions = [0, 1, 2, 3, -1]
    ...
    def move(self, action):
        ...
        if action == 3:  # right
            return self.player_location[0], self.player_location[1] + 1
        if action == -1: # none
            return self.player_location[0], self.player_location[1]
    ...
    def play(self):
        self.reset()
        user_input_map = {'w': 0, 's': 1, 'a': 2, 'd': 3, 'e': -1}
