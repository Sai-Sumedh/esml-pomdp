...
def update_pi(pi_curr, p_theta, Onew_ind, numvals_z):
    #Z_prev_index is not needed- will be removed in future versions of this function
...
def update_theta_es(theta_old, p_theta_old, a_t, omega_t, pi_t, Onew_ind, numvals_z, theta_indices, theta_not_indices, gamma=0.1):
    ...
    #update the value of theta using extremum seeking
    ...
    # prod_pi_ptheta = np.tensordot(pi_t, p_theta_pert[:,:,Onew_ind,:,:,:], axes=([0,1],[2,3])) #marginalise over X_n
    # p_th_pert_cond = np.sum(prod_pi_ptheta.flatten())*(1/numvals_z) #marginalize over X_(n+1) and Z_n
    thr = 1e-5
    # if p_th_pert_cond<=0:
    #     print(f"{p_theta_pert[theta_indices]}")
    #     print(f"{p_theta_pert[theta_not_indices]}")
    #     print(f"{np.amax(p_theta_pert)}")
    #     print(f"pi_t sum:{np.sum(pi_t)}")
    #     print('term inside log is zero')
    #     print(f"O={Onew_ind}")
    #     print(p_theta_pert_marg)
    #     print(p_th_pert_cond)
    #     print(pi_t)
...
def estimate_theta_es_gridworld(theta_init, theta_star, T=500000, alpha = 1, beta = 0.8, eps = 0.1, \
    ...
    for t in range(1, T):
        ...
        theta_hat_t[t] = update_theta_es(theta_old=theta_hat_t[t-1], p_theta_old=p_theta_hat, a_t=a_t, omega_t=omega_t, pi_t=pi_t, Onew_ind=Ot, \
...
def compute_log_likelihood_alldata(theta_estimate, O_t, Z_t, Tmax, env, rule_new_details, saveLLt=False):
    ...
    for t in range(1,Tmax):
        ...
        pi_t = update_pi(pi_t, p_theta, int(O_t[t]), numvals_z=len(action_space))
        LL_t[t] = LL
    if saveLLt:
        return LL_t
    else:
